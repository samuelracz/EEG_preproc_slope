function [ f ] = func_plot_EO_EC_spectra(data_eo, data_ec, filt_list, ch_list)
...
for ch = 1:nch
    for ftype = 1:nf
        ...
        mu_mixd_eo = exp(mean(log(tmp_eo.mixd),2));
        mu_frac_eo = exp(mean(log(tmp_eo.frac),2));
        std_pos_eo = exp(mean(log(tmp_eo.mixd),2) + std(log(tmp_eo.mixd),[],2));
        std_neg_eo = exp(mean(log(tmp_eo.mixd),2) - std(log(tmp_eo.mixd),[],2));
        mu_plaw_lo_eo = exp(mean(log(tmp_eo.Plaw_lo),2));
        mu_plaw_hi_eo = exp(mean(log(tmp_eo.Plaw_hi),2));
        mu_mixd_ec = exp(mean(log(tmp_ec.mixd),2));
        mu_frac_ec = exp(mean(log(tmp_ec.frac),2));
        std_pos_ec = exp(mean(log(tmp_ec.mixd),2) + std(log(tmp_ec.mixd),[],2));
        std_neg_ec = exp(mean(log(tmp_ec.mixd),2) - std(log(tmp_ec.mixd),[],2));
        mu_plaw_lo_ec = exp(mean(log(tmp_ec.Plaw_lo),2));
        mu_plaw_hi_ec = exp(mean(log(tmp_ec.Plaw_hi),2));

        subplot(nch,nf,ind_sp)
        loglog(freq_eo, mu_mixd_eo, 'r')
        hold on
        loglog(freq_ec, mu_mixd_ec, 'b')
        ...
        % means
        pm_eo = loglog(freq_eo, mu_mixd_eo, ':', 'color', [1 0.5 0.5], 'LineWidth' ,3);
        loglog(freq_eo, mu_frac_eo, '-', 'color', [0.7 0 0], 'LineWidth' ,3);
        loglog(freq_lo_eo, mu_plaw_lo_eo, 'r--', 'LineWidth',3);
        loglog(freq_hi_eo, mu_plaw_hi_eo, 'r--', 'LineWidth',3);
        pm_ec = loglog(freq_ec, mu_mixd_ec, ':', 'color', [0.5 0.5 1], 'LineWidth' ,3);
        loglog(freq_ec, mu_frac_ec, '-', 'color', [0 0 0.7], 'LineWidth' ,3);
        loglog(freq_lo_ec, mu_plaw_lo_ec, 'b--', 'LineWidth',3);
        loglog(freq_hi_ec, mu_plaw_hi_ec, 'b--', 'LineWidth',3);
